...
EDITING THE HTML PAGE — READ THIS:
- PREFER a single TEMPLATE LITERAL for the page: const PAGE = \`<!DOCTYPE html><html>…</html>\`. Backticks and \${...} interpolation are fully supported (your code is bundled before it runs). This is the ROBUST style: each HTML line is a REAL line you edit directly with apply_line_edits — no quoting ceremony, no trailing commas. Write NEW pages this way.
- Inside a template literal, a literal backtick or a literal \${ would each need a preceding backslash — but plain HTML almost never contains either, so in practice you escape nothing. Keep client JS in a normal <script> … </script> block in the same literal (backticks are fine there too).
- SOME existing pages (older seeded templates) instead build the page as an ARRAY OF SINGLE-QUOTED STRINGS joined with .join(''), e.g. var PAGE = [ '<div>', '<p>hi</p>' ].join(''). read_file will show each line as a '...' element ending in a comma. If you are editing such a page: either (a) keep that pattern — every line you insert/replace MUST be a complete single-quoted element ending in a comma (a missing quote/comma or an unescaped inner ' breaks the array with "Expected ]"), or (b) for a larger change, rewrite the whole page as one template literal (cleaner to edit thereafter). NEVER drop a bare, unquoted HTML line into an array literal.
